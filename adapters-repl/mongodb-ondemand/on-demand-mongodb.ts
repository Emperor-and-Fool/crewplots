...
export class OnDemandMongoService {
  ...
  constructor() {
    console.log(
      "[OnDemandMongo] Service initialized, Docker mode:",
      this.dockerMode,
    );
  }
  ...
  async ensureReady(): Promise<boolean> {
    if (this.dockerMode) {
      console.log(
        "[OnDemandMongo] Docker mode - skipping local service activation",
      );
      return false;
    }
  ...
  private async start(): Promise<boolean> {
    ...
    try {
      ...
      this.mongoProcess.on("exit", (code, signal) => {
        console.log(
          `[OnDemandMongo] Process exited with code ${code}, signal ${signal}`,
        );
        this.cleanup();
      });
