...
void queue_response(Client *client, const char *response) {
    ...
    if (client->response_len + len < sizeof(client->response_queue)) {
...
int parse_resp_command(const char *buffer, int buffer_len, char **args, int max_args, int *consumed) {
    printf("DEBUG: Parsing buffer of length %d\n", buffer_len);
    printf("DEBUG: Buffer content: ");
    for (int i = 0; i < buffer_len && i < 20; i++) {
        if (buffer[i] == '\r') {
            printf("\\r");
        } else if (buffer[i] == '\n') {
            printf("\\n");
    ...
    if (buffer_len < 4 || buffer[0] != '*') {
        printf("DEBUG: Invalid header or too short\n");
        return -1;
    }
    ...
    if (header_end == -1) {
        printf("DEBUG: Incomplete header - no \\r\\n found in positions 1 to %d\n", buffer_len - 2);
        for (int i = 0; i < buffer_len && i < 10; i++) {
            printf("DEBUG: buffer[%d] = %d (0x%02x)\n", i, (unsigned char)buffer[i], (unsigned char)buffer[i]);
        }
        return -1;
    }
    ...
    for (int i = 0; i < argc; i++) {
        printf("DEBUG: Processing argument %d at pos %d\n", i, pos);
        if (pos >= buffer_len || buffer[pos] != '$') {
            printf("DEBUG: Invalid argument header at pos %d\n", pos);
            return -1;
        }
        ...
        if (len_end == -1) {
            printf("DEBUG: Incomplete length for arg %d\n", i);
            return -1;
        }
        
        int arg_len = atoi(buffer + pos + 1);
        printf("DEBUG: arg_len = %d\n", arg_len);
        ...
        if (pos + arg_len + 2 > buffer_len) {
            printf("DEBUG: Incomplete data for arg %d\n", i);
            return -1;
        }
...
void process_command(Client *client, char **args, int argc) {
    ...
    printf("DEBUG: Command (uppercase): '%s'\n", cmd);
    
    if (strcmp(cmd, "PING") == 0) {
        printf("DEBUG: Processing PING command\n");
        if (argc > 1) {
            // PING with message
            char response[1024];
            ...
            printf("DEBUG: Sending PONG response\n");
...
void handle_client_data(Client *client) {
    printf("DEBUG: Handling client data, buffer_len = %d\n", client->buffer_len);
    ...
    while (pos < client->buffer_len) {
        ...
        if (argc <= 0) {
            printf("DEBUG: Parse failed or incomplete command\n");
    ...
    while (1) {
        ...
        if (FD_ISSET(server_fd, &read_fds)) {
            ...
            if (client_fd >= 0 && client_count < MAX_CLIENTS) {
                printf("DEBUG: New client connected: %d\n", client_fd);
        ...
        for (int i = 0; i < client_count; i++) {
            ...
            if (FD_ISSET(clients[i].fd, &read_fds)) {
                printf("DEBUG: Reading from client %d\n", clients[i].fd);
                int bytes_read = read(clients[i].fd, 
                                    clients[i].buffer + clients[i].buffer_len,
                                    sizeof(clients[i].buffer) - clients[i].buffer_len - 1);
