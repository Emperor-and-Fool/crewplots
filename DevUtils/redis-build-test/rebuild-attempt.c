...
#define BUFFER_SIZE 16384     // Doubled from 8192
#define MAX_ARGS 32
#define MAX_KEY_SIZE 1024     // Doubled from 512
#define MAX_VALUE_SIZE 4096   // Doubled from 2048
...
// Fixed RESP parser with increased buffer limits
int parse_resp_command(const char *buffer, int buffer_len, char **args, int max_args, int *consumed) {
    *consumed = 0;
    ...
    for (int i = 0; i < argc; i++) {
        ...
        if (len_end == -1) return -1;
        
        int arg_len = atoi(buffer + pos + 1);
        if (arg_len < 0 || arg_len >= MAX_VALUE_SIZE) return -1; // Increased limit check
        
        pos = len_end + 2;
        
        if (pos + arg_len + 2 > buffer_len) return -1;
