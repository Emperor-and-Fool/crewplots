...
export default function SchedulerCreatePage() {
  ...
  const createShiftMutation = useMutation({
    mutationFn: async (data: ShiftCreationForm) => {
      const shifts = data.daysOfWeek.map(day => ({
      ...
      return apiRequest('POST', `/api/week-schedules/${currentWeekSchedule.id}/shifts`, { shifts });
    },
    onSuccess: () => {
  ...
  const handleShiftClick = (shift: any) => {
    ...
    // Populate shift form with clicked shift data
    shiftForm.reset({
                  ...
                  <CardContent className="space-y-2">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        {locationsData.find((l: Location) => l.id === currentWeekSchedule?.locationId)?.name}
                      </p>
                    </div>
                  ...
                  <CardContent>
                    <div className="space-y-2">
                      <div className="flex items-center justify-between text-sm">
                        ...
                        <Badge variant="secondary">{shiftsData.length}</Badge>
                      </div>
