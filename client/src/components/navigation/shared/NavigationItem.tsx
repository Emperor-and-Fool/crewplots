...
export function NavigationItem({ 
  ...
  return (
    ...
    >
      {/* Handle items with children (nested structure) - render as simple list to avoid nested accordions */}
      {item.children && item.children.length > 0 ? (
        <div>
          <div className={cn(
            "w-full flex items-center text-sm font-medium",
            layout === 'desktop' 
              ? "py-1 text-primary-200"
              : "px-2 py-2 rounded-md text-primary-200"
          )}>
            {IconComponent && (
              <IconComponent 
                className={cn(
