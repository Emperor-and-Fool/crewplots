...
const messagingBusinessRules = [
  (data: any, context: any) => {
    ...
    // Skip content validation for read operations
  ...
  (data: any, context: any) => {
    ...
    // Skip user context validation for read operations entirely
    if (context?.operation === 'read') {
      return { warnings, errors }; // Allow all read operations 
    }
    ...
    const userId = contextUser?.id || context?.userId;
    const userRole = contextUser?.role || context?.role;
