...
export class DatabaseStorage implements IStorage {
  ...
  async getUsersByRole(role: string): Promise<User[]> {
    ...
    if (role === 'staff') {
      ...
      roleUsers = await db.select().from(users).where(inArray(users.role, ['crew_member'])).orderBy(asc(users.name));
    } else if (role) {
      roleUsers = await db.select().from(users).where(eq(users.role, role)).orderBy(asc(users.name));
    } else {
      roleUsers = await db.select().from(users).orderBy(asc(users.name));
    }
